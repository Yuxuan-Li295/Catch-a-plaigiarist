...
public class DocumentsProcessorTest {
	...
	String dir = "/autograder/submission/test files";
	...
	public void ProcessDocuments() {
		...
		testmap = processor.processDocuments(dir, 2);
		assertEquals(testlist, testmap.get(file1));
	}

	@org.junit.Test
	public void teststoreNWordSequences1() {
		testmap = processor.processDocuments(dir, 4);
	...
	public void computeSimilarities() {
		...
		for (Similarities sb : treeset2) {
			...
			if (sb.getFile1().equals(file5)
					&& sb.getFile2().equals(file4)) {
	...
	public void processAndStore() {
		tuplelistTest = processor.processAndStore(dir,sequenceFile, 4);
		int size = 0;
		for (Tuple<String, Integer> tuple : tuplelistTest) {
			if (tuple.getLeft().equals(file1)) {
				size = tuple.getRight();
